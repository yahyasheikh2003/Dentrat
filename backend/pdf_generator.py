...
def generate_analysis_pdf(analysis: dict) -> bytes:
    ...
    story.append(Paragraph("AI Diagnostic Report", subtitle_style))
    ...
    if detections:
        table_data = [["#", "Condition", "Tooth", "Severity", "Confidence"]]
        for i, det in enumerate(detections, 1):
            table_data.append(
                [
                    str(i),
                    det.get("class", "Unknown"),
                    det.get("tooth", det.get("location", "—")),
                    det.get("severity", "—"),
        ...
        results_table = Table(
            table_data,
            colWidths=[0.35 * inch, 1.8 * inch, 1.8 * inch, 1.1 * inch, 0.95 * inch],
        )
    ...
    story.append(
        Paragraph("This is an AI-generated report, please consult your doctor.", footer_style)
    )
